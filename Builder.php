...
abstract class Builder
{
    ...
    protected $insertByKeySql   = '%INSERT% INTO %TABLE% (%FIELD%) VALUES (%DATA%) %COMMENT% ON DUPLICATE KEY UPDATE %ONKEY%';
    ...
    //生成更新语句
    public function insertByKey(array $data, $options = [], $keyarr=[])
    {
        // 分析并处理数据
        $data = $this->parseData($data, $options);
        if (empty($data)) {
        ...
        $fields2=array_keys($keyarr);
        $values2=array_values($keyarr);
        $sql = str_replace(
            ['%INSERT%', '%TABLE%', '%FIELD%', '%DATA%', '%COMMENT%','%ONKEY%'],
            [
                'INSERT',
                $this->parseTable($options['table'], $options),
                implode(' , ', $fields),
